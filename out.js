...
(() => {
  ...
  // scripts/utils.js
  var isUniqTask = (newTaskText) => tasks.every(({ text }) => newTaskText !== text);

  // scripts/localTasks.js
  var import_isEqual = __toESM(require_isEqual());
  var addTaskToLocalStorage = (task) => {
    const localStorage = window.localStorage;
    const localTasks = localStorage.getItem("tasks");
    const tasks2 = localTasks ? JSON.parse(localTasks) : [];
    tasks2.forEach((localTask) => {
      if ((0, import_isEqual.default)(localTask, task))
        return;
      console.log("test after return in forEach");
    });
    tasks2.push(task);
    localStorage.setItem("tasks", JSON.stringify(tasks2));
    console.log("test after return after forEach");
  };
  var deleteTaskFromLocalStorage = (taskId) => {
    const localStorage = window.localStorage;
    const tasks2 = JSON.parse(localStorage.getItem("tasks"));
    const deleteTaskIdIndexInTasks = tasks2.findIndex((task) => task.id === taskId);
    deleteTaskIdIndexInTasks >= 0 ? tasks2.splice(deleteTaskIdIndexInTasks, 1) : tasks2.splice(deleteTaskIdIndexInTasks, deleteTaskIdIndexInTasks);
    localStorage.setItem("tasks", JSON.stringify(tasks2));
  };

  // scripts/createTask.js
  ...
  var createTask = (text, id = Date.now(), completed = false) => {
    const stringId = String(id);
    const task = { id: stringId, completed, text };
    addTaskToLocalStorage(task);
    return task;
  };
  var createErrorMessage = (errText) => {
    const errorSpan = document.createElement("span");
    errorSpan.classList.add("error-message-block");
    errorSpan.textContent = errText;
    return errorSpan;
  };
  var newTaskForm = document.querySelector(".create-task-block");
  newTaskForm.addEventListener("submit", (event) => {
    ...
    if (!isUniqTask(inputText)) {
      const errBlock2 = createErrorMessage("\u0417\u0430\u0434\u0430\u0447\u0430 \u0441 \u0442\u0430\u043A\u0438\u043C \u043D\u0430\u0437\u0432\u0430\u043D\u0438\u0435\u043C \u0443\u0436\u0435 \u0435\u0441\u0442\u044C.");
      newTaskForm.append(errBlock2);
    } else if (inputText === "") {
      const errBlock2 = createErrorMessage("\u041D\u0430\u0437\u0432\u0430\u043D\u0438\u0435 \u043D\u0435 \u0434\u043E\u043B\u0436\u043D\u043E \u0431\u044B\u0442\u044C \u043F\u0443\u0441\u0442\u044B\u043C.");
      newTaskForm.append(errBlock2);
    } else {
      const newTask = createTask(inputText);
      tasks.push(newTask);
      const newTaskElement = createTaskElement(newTask);
      const taskList2 = document.querySelector(".tasks-list");
      taskList2.append(newTaskElement);
      event.target.taskName.value = null;
    }
  });

  // scripts/modalDelete.js
  var createModalConfirmElement = () => {
  ...
  var modalDeleteHandler = function(event) {
    ...
    deleteModalButtons.addEventListener("click", (modalEvent) => {
      ...
      if (modalClickTarget === deleteBtn_Modal) {
        ...
        deleteTaskIdIndexInTasks >= 0 ? tasks.splice(deleteTaskIdIndexInTasks, 1) : tasks.splice(deleteTaskIdIndexInTasks, deleteTaskIdIndexInTasks);
  ...
  var changeTheme = () => {
    ...
    const tasks2 = document.querySelectorAll(".task-item");
    tasks2.forEach((task) => task.classList.toggle("item-dark"));
